...
type OxiWasmModule = {
  ...
  OxiHumanEngine: {
    from_core_pack_bytes: (bytes: Uint8Array) => OxiEngine;
  };
};
...
function parseMeasurements(json: string): OxiMeasurements {
  ...
  return {
    heightCm: numberOrNull(raw.height_cm, raw.height, raw.stature_cm, raw.stature),
...
export class OxiHumanScene {
  ...
  private constructor(
    ...
    const rim = new THREE.DirectionalLight(0xc4b5fd, 1.45);
    rim.position.set(-2, 6, -8);
    this.scene.add(rim);

    this.scene.add(new THREE.HemisphereLight(0xe4e4ff, 0x17171e, 0.78));

    const floorGeometry = new THREE.PlaneGeometry(4000, 4000);
    floorGeometry.rotateX(-Math.PI / 2);
    this.floor = new THREE.Mesh(floorGeometry, new THREE.ShadowMaterial({ opacity: 0.22 }));
    this.floor.receiveShadow = true;
    this.scene.add(this.floor);

    this.grid.material.transparent = true;
    this.grid.material.opacity = 0.23;
  ...
  static async create(canvas: HTMLCanvasElement, onProgress?: (progress: OxiLoadProgress) => void) {
    ...
    try {
      ...
      throw new Error(`Не удалось загрузить OxiHuman WASM: ${error instanceof Error ? error.message : String(error)}`);
    }

    onProgress?.({ phase: 'pack', progress: 0.2, label: 'Загружаю антропометрический core pack' });
    const pack = await fetchBytes(packUrl, (progress) => {
      onProgress?.({ phase: 'pack', progress: 0.2 + progress * 0.68, label: 'Загружаю антропометрический core pack' });
    });

    onProgress?.({ phase: 'mesh', progress: 0.92, label: 'Собираю настоящий mesh' });
    const engine = wasm.OxiHumanEngine.from_core_pack_bytes(pack);
    const memory = wasm.wasm_memory();
    const scene = new OxiHumanScene(canvas, engine, memory, wasm.get_version());
    onProgress?.({ phase: 'mesh', progress: 1, label: 'OxiHuman готов' });
    return scene;
  }
  ...
  getVertexCount() {
    return this.engine.get_vertex_count();
  }

  getIndexCount() {
    return this.engine.get_index_count();
  }
  ...
  preview(params: BodyParams) {
    ...
    const weightPrior = clamp(bmiPrior * 0.7 + fatPrior * 0.3, 0, 1);

    this.engine.set_param('height', heightPrior);
    this.engine.set_param('weight', weightPrior);
  ...
  async fit(params: BodyParams): Promise<OxiFitResult> {
    ...
    try {
      ...
      throw new Error(`OxiHuman fit failed: ${error instanceof Error ? error.message : String(error)}`);
    }

    // The fitter is allowed to move macro gender/muscle. CharacterMaker treats
    // these as explicit user parameters, so re-apply them after the centimetre fit.
    this.engine.set_param('gender', params.gender === 'female' ? 1 : 0);
    this.engine.set_param('muscle', clamp(params.muscle / 3, 0, 1));
    this.refreshGeometry(true);
    ...
    // If a build exposes only fit report dimensions, use those as a fallback.
  ...
  private makeViews() {
    ...
    this.indices = new Uint32Array(buffer, this.engine.indices_ptr(), this.engine.indices_len());
  }
